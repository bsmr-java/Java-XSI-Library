...
public class BumpMapEffect
{
    ...
    // this should be calculated on graphiccard if I understand that piece of
    // code right
    ...
    public void bumpMap()
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                bumpX[x][y] = (rgb[Math.min(w - 1, x + 1) + y * w] & 255) - (rgb[Math.max(0, x - 1) + w * y] & 255);
                bumpY[x][y] = (rgb[x + w * Math.min(h - 1, y + 1)] & 255) - (rgb[x + w * Math.max(0, y - 1)] & 255);
            }
        }
    }
    ...
    public void paintComponent(Graphics graphics)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                px = Math.max(0, Math.min(255, -bumpX[x][y] + 127 + (x - lichtX) / 3));
                py = Math.max(0, Math.min(255, -bumpY[x][y] + 127 + (y - lichtY) / 3));
                f = map[px][py];
